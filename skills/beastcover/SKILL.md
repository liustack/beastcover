...
Use an installed `beastcover` command when available. Otherwise prefix each command with `npx --yes --package @liustack/beastcover@0.3.3`.

Run `beastcover doctor` once on a new machine. If it reports Chromium missing, run `npx --yes --package @liustack/beastcover@0.3.3 playwright install chromium`, then run doctor again. Do not run a bare `npx playwright install`: it can resolve a different Playwright version and download a browser this package cannot use.
...
```bash
npx --yes --package @liustack/beastcover@0.3.3 beastcover gen "<headline>" --source render --preset youtube --output <path>.png
```
