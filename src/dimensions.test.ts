...
describe('dimension presets', () => {
    it('exposes one preset per platform at its production pixel size', () => {
        ...
        expect(getDimensionPreset('wechat')).toEqual({
            name: 'wechat',
            width: 900,
            height: 383,
    ...
    it('names the replacement when a retired ratio preset is used', () => {
        expect(() => getDimensionPreset('16:9')).toThrowError('Preset "16:9" is now "youtube".');
        expect(() => getDimensionPreset('5:2')).toThrowError(
            'Preset "5:2" is gone. Use "x" or "wechat" for a wide banner cover.',
        );
        expect(() => getDimensionPreset('3:2')).toThrowError(
            'Preset "3:2" is gone. Use "youtube" or "bilibili" for a landscape cover.',
        );
        expect(() => getDimensionPreset('3:4')).toThrowError(
    ...
    it('lists every preset for an unknown name, including inherited object keys', () => {
        ...
        expect(() => getDimensionPreset('square')).toThrowError(
            `Unknown dimension preset "square". ${message}`,
        );
        expect(() => getDimensionPreset('toString')).toThrowError(
            `Unknown dimension preset "toString". ${message}`,
        );
    });
});
