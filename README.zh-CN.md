...
不要照片的话，`--source render` 出纯文字封面。`--template poster` 是大字报：满版纯色、超大字，可以用 `--tag` 加一个痛点标签。`--template number --number 3` 是数字钩子：一个超大数字配一句短话。`--template compare --before 旧.jpg --after 新.jpg` 是前后对比：两张图分屏，接缝处一个箭头。每个模板都会按各平台的安全区排版。视频封面要的是几个字的钩子，公众号和 X 文章的卡片上标题就挨着封面，所以 `--hook "封面没人点？"` 会把短句放在视频和笔记封面上，公众号和 X 保留完整标题。装了模型 CLI 的话，`--source local-model --via codex` 按项目风格画封面，`beastcover styles` 列出四种画风。
